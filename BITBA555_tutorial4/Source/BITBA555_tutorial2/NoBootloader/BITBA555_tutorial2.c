...
{
  volatile uint16_t tablePos;  // tablePos determines where in the waveform table the voice is currently
  uint8_t midiNote; // midiNote records the current midi note value
  uint8_t ampVal; // ampVal determines the amplitude value to apply to the note (should be 0 to 16)
  ...
  volatile uint8_t incrTablePos;  // a value used to increment through the incrTable
  ...
  uint8_t numNotesPressed;  // the number of previous notes still being held
  uint8_t incrTableNeedsProc; // 1 if the incrTable needs to be reprocessed, 0 if it doesn't
} Voice;
...
ISR (TIMER1_COMPA_vect)
{
  // Use this for tutorial before implementing MIDI notes
  /*
  SPIDAC_MasterTransmit ( currentWave[timerWaveIncr] * 16 );

  timerWaveIncr = (timerWaveIncr + 1) % INITIAL_WAVE_TABLE_SIZE;
  */
